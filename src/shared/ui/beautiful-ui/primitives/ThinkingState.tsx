...
export default function ThinkingState({
  ...
  const live = workingProp !== undefined || text !== undefined;
  const stage = useSequence(live ? [0] : STAGES);
  const [manualExpanded, setManualExpanded] = useState<boolean | null>(null);
  const [selectedTool, setSelectedTool] = useState<string | null>(null);
  const liveRows = text ? [{ primary: text }] : rows;
  const v = {
  ...
  return (
    <div
      key={variant}
      className={`flex w-full flex-col ${live ? "" : "max-w-95"}`}
      style={{
        minHeight: live ? undefined : working || expanded ? 176 : undefined,
        ...
        <div className="overflow-hidden">
          <div className="relative mt-1 ml-[5px] pl-4">
            ...
            {v.rows.slice(0, visible).map((row, i) => {
              ...
              const rowClass = `flex w-full items-start gap-2 rounded-[6px] px-1.5 py-0.5 text-left ${variant === "Reasoning" ? "min-h-0" : "min-h-7 items-center"}`;
              ...
              return (
                <div key={live ? `live-${i}` : row.primary} className={rowClass} style={animation}>
                  {content}
                </div>
              );
            })}
