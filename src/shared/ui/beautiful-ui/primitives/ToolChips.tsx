...
const STEP_MS = 700;
/** Ignore tools that finish before the expand animation would even be visible. */
...
export default function ToolChips({
  ...
  summary?: string;
  /** Keep the group expanded until the reply body starts (or the turn ends). */
  holdOpen?: boolean;
}) {
  const rootRef = useRef<HTMLDivElement>(null);
  const [step, setStep] = useState(0);
  const liveKeys = (calls ?? [])
    .filter((call) => call.status === "running" || call.status === "pending")
    .map((call) => call.id ?? call.label);
  const liveKeySig = liveKeys.join("|");
  ...
  useEffect(() => {
    if (calls || step >= total) return;
    const t = setTimeout(() => setStep((s) => s + 1), STEP_MS);
    return () => clearTimeout(t);
  }, [calls, step, total]);

  const liveMode = Boolean(calls);
  useEffect(() => {
    if (!liveMode) return;
    const keys = liveKeySig ? liveKeySig.split("|") : [];
    if (keys.length > 0) {
      const delay = openRef.current ? 0 : OPEN_DELAY_MS;
      const timer = window.setTimeout(() => {
        setOpen(true);
        setOpenRows(new Set(keys));
      }, delay);
  ...
  return (
    <div ref={rootRef} className={`${calls ? "min-h-0 w-full" : "min-h-[220px] w-full max-w-80"} pb-1`}>
      ...
      >
        ...
        <span className="tabular-nums">{summary ?? "4 tool calls, 2 messages"}</span>
      </button>
      ...
      <div className="grid transition-[grid-template-rows,opacity] duration-300" style={{ gridTemplateRows: open ? "1fr" : "0fr", opacity: open ? 1 : 0 }}>
        ...
        <div className="mt-1.5 flex flex-col gap-1">
          {(calls
            ? calls.map((call) => ({
                key: call.id ?? call.label,
                icon: call.icon ?? "observe",
                label: call.label,
                chip: call.chip,
                mono: Boolean(call.mono),
                status: call.status,
                detailMono: call.detailMono ?? Boolean(call.mono),
                detail: (call.detail ?? []).map((text): DetailLine => ({ text })),
              }))
            ...
            <div
              key={row.key}
              style={calls ? undefined : { animation: "fade-up 300ms cubic-bezier(0.23,1,0.32,1) both" }}
            >
              <button
                type="button"
                ...
                <div className="min-h-0 overflow-hidden">
                  <div className="mt-0.5 mb-1 ml-2 flex min-w-0 flex-col gap-0.5 border-l border-line py-0.5 pl-3.5">
                    {row.detail.length > 0 ? row.detail.slice(0, 1).map((line, lineIndex) => (
                      ...
                      <span className="block min-w-0 truncate text-[11.5px] leading-[1.6] text-ink-3">暂无详情</span>
                    )}
                  </div>
                </div>
              </div>
